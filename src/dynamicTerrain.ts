...
export class DynamicTerrain {
    ...
    constructor(name: string, options: {
        ...
        this._noiseScale = options.noiseScale || 0.01;
        ...
        this._mapData = new Float32Array(this.mapSubX * this.mapSubZ * 3); 
        ...
        for (var l = 0; l < this.mapSubZ; l++) {
            for (var w = 0; w < this.mapSubX; w++) {
              
              const noise = noise2D(l * this._noiseScale, w * this._noiseScale);
        
              const x = (w - this.mapSubX * 0.5) * 2.0;
              const y = noise * noise * 10
              const z = (l - this.mapSubZ * 0.5) * 2.0
        
              this.mapData[3 * (l * this.mapSubX + w)] = x;
              this.mapData[3 * (l * this.mapSubX + w) + 1] = y;
              this.mapData[3 * (l * this.mapSubX + w) + 2] = z;
           }       
        }
